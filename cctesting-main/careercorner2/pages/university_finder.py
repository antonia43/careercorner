...
def render_university_finder():
    """Main university finder with Portuguese and International modes"""
    ...
    mode = st.radio(
        ...
        ["Portugal (DGES Data)", "International (AI Search)"],
        horizontal=True,
        key="university_finder_mode"
    )

    if mode.startswith("Portugal"):
...
def render_portuguese_finder():
    ...
    if not st.session_state.university_pt_gate_done:
        st.info(
            "This tool uses DGES data for Portuguese universities. "
            "It only works if you are planning to study in Portugal and comparing Portuguese degrees."
        )
        ...
        with col2:
            if st.button(
                "No, I'm not applying in Portugal",
                width='stretch',
                key="uf_pt_no",
            ):
                st.warning(
                    "University Finder currently only supports Portuguese universities for DGES data. "
                    "Please use International mode for other countries."
                )
        return

...
def render_grades_analysis():
    ...
    # choosing input method (if no grades yet)
    if st.session_state.student_grades_data is None:
        if st.session_state.grades_input_method is None:
            ...
            is_portuguese = st.session_state.get("is_portuguese_student", True)
            if is_portuguese:
                render_manual_grades_input()
            else:
                render_international_grades_input()
        elif st.session_state.grades_input_method == "file":
...
def render_input_method_selection():
    """Choosing between manual entry or file upload"""
    st.markdown("---")
    st.subheader("Step 1: Where are you from?")
    
    student_location = st.radio(
        "Are you a Portuguese student or an international student?",
        ["🇵🇹 Portuguese Student (Portuguese school system)", 
         "🌍 International Student (Other school system)"],
        key="student_location_radio"
    )
    
    is_portuguese = "Portuguese" in student_location
    
    st.markdown("---")
    st.subheader("Step 2: How will you input your grades?")
    ...
    with col1:
        if is_portuguese:
            st.markdown("""
            #### ✍︎ Manual Entry (Portuguese)
            - Guided Portuguese grade entry
            - Select your track and subjects
            - We'll handle DGES calculations
            - ~5 minutes
            """)
        else:
            st.markdown("""
            #### ✍︎ Manual Entry (International)
            - Add all subjects manually
            - Enter grades in your system
            - Custom grade scales
            - ~5 minutes
            """)
        
        if st.button("✍︎ Manual Entry", width='stretch', type="primary", key="btn_manual_entry_start"):
            st.session_state.grades_input_method = "manual"
            st.session_state.is_portuguese_student = is_portuguese
            st.rerun()
    
    with col2:
        st.markdown("""
        #### 🗂️ Upload Document
        - Upload transcript/grades sheet
        - Automatic extraction
        - Works for any country
        - ~2 minutes
        """)
        
        if st.button("➜] Upload File", width='stretch', key="btn_file_upload_start"):
            st.session_state.grades_input_method = "file"
            st.session_state.is_portuguese_student = is_portuguese
            st.rerun()
    
    if not is_portuguese:
        st.info("ⓘ **Note for International Students:** We'll help you analyze your grades, but university recommendations will be general advice rather than specific universities.")
...
def render_file_upload_grades():
    """Uploading and extracting grades from file"""
    
    st.subheader("🗂️ Upload Your Grades")
    
    if st.button("← Back to Input Method Selection", key="btn_back_from_file_upload"):
        st.session_state.grades_input_method = None
        st.rerun()
    
    st.markdown("---")
    
    st.info("ⓘ **Tip:** Upload a clear photo or PDF of your transcript. Works for any country!")
    
    uploaded_file = st.file_uploader(
        "Upload your transcript or grades sheet",
        type=["pdf", "jpg", "png", "jpeg"],
        help="We'll extract your grades automatically"
    )
    ...
    if uploaded_file:
        ...
        if file_extension in ['jpg', 'jpeg', 'png']:
            st.image(temp_filename, caption="Uploaded Transcript", width='stretch')
        
        st.markdown("---")
        
        if st.button("⟡ Extract Grades", width='stretch', type="primary", key="btn_extract_grades"):
            with st.spinner("We're analyzing document and extracting grades... Hold on tight!"):
                extracted_data = extract_grades_from_file(temp_filename)
            ...
            if extracted_data:
                ...
                if extracted_data.get("input_method") == "file":
                    st.json(extracted_data)
                    st.session_state.is_portuguese_student = True
                else:
                    st.write(f"**Country:** {extracted_data.get('country', 'Unknown')}")
                    st.write(f"**Grading Scale:** {extracted_data.get('grade_scale', 'Unknown')}")
                ...
                with col1:
                    if st.button("✓ Confirm & Continue", width='stretch', type="primary", key="btn_confirm_extracted"):
                        ...
                        if extracted_data.get("input_method") == "file":
                            st.session_state["gradesdata"] = {
                                "inputmethod": "manual",
                                "grades": extracted_data.get("grades", {}),
                            }
                        else:
                            st.session_state["gradesdata"] = {
                                "inputmethod": extracted_data.get("input_method", "fileinternational"),
                ...
                st.error("⃠ Could not extract grades from this file.")
...
def extract_grades_from_file(file_path: str):
    """Extract grades from uploaded file using Gemini + Langfuse logging."""
    ...
    try:
        ...
        prompt = """
Analyze this transcript/grades document and extract ALL grades.

For PORTUGUESE students, return JSON like:
{
  ...
  "grades": {
    "10th": {"Português": 15, "Matemática": 17, ...},
    "11th": {...},
    "12th": {...},
...
For INTERNATIONAL students, return JSON like:
{
  "student_type": "international",
  "country": "country name or 'Unknown'",
  "grade_scale": "e.g., 0-100, A-F",
  "subjects": [
    {"name": "Mathematics", "grade": "95", "year": "11th Grade"},
    {"name": "Physics", "grade": "A", "year": "12th Grade"}
  ],
  "current_year": "best guess",
  "school_type": "if visible"
}

Return ONLY valid JSON. Do NOT include prose. Be thorough.
"""
        ...
        if data.get("student_type") == "portuguese":
            ...
            return {
                "input_method": "file_international",
        ...
        st.error(f"⚠︎ Could not extract grades from file: {e}")
        st.warning("⚠︎ Please try manual entry instead, or check if your file is readable.")
        return None
...
def render_international_grades_input():
    ...
    if st.button("← Back to Input Method Selection", key="btn_back_from_intl_entry"):
        st.session_state.grades_input_method = None
        st.rerun()
    ...
    if st.session_state.temp_intl_grades:
        ...
        for i, subj in enumerate(st.session_state.temp_intl_grades):
            ...
            if st.button("🗑️", key=f"del_intl_{i}"):
        ...
        st.info("↑ Add your first subject above!")

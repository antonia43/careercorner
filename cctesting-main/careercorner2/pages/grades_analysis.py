...
def render_input_method_selection():
    st.markdown("---")
    st.info("As of now, grades analysis is only available for portuguese students!")
    st.markdown("---")
    ...
    if st.button("✍︎ Go to Entry", width='stretch', type="primary", key="btn_manual_entry_start"):
...
def extract_grades_from_file(file_path: str):
    ...
    try:
        ...
        # Base64 encode for Gemini vision
...
"""
        ...
        st.error("⚠︎ Could not parse JSON response from Gemini.")
        return None
    except Exception as e:
